...
@Component({
  selector: 'app-user-performance-panel',
  standalone: true,
  imports: [CommonModule, FormsModule, CardComponent, ChartWrapperComponent, DataTableComponent],
  templateUrl: './user-performance-panel.component.html',
})
export class UserPerformancePanelComponent {
  ...
  filteredRecords = computed(() => {
    ...
    if (userEmail) {
      ...
      filteredByUser = records.filter((record) => {
        ...
        if (!matches && records.length > 0 && records.indexOf(record) === 0) {
          console.log('User Performance Panel - Email mismatch:', {
      ...
      console.log('User Performance Panel - Filtered records:', filteredByUser.length, 'out of', records.length);
    } else {
      console.warn('User Performance Panel - No user email found, showing all records');
    }
    ...
    var data = filteredByUser.filter((record) => {
